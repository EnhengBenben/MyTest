...
function getCode() {
    ...
    xml.open("post", "/wx/code?phone_number=" + tel.value, true);
    xml.send();
...
function login() {
    ...
    xml.open("post", "/wx/sign_up?phone_number=" + tel.value + "&password=" + password.value + "&code="+code.value, true);
    xml.send();
    xml.onreadystatechange = function () {
        if (xml.readyState == 4)
            if (xml.status == 200) {
